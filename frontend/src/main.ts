...
import { setBenefitTokenProvider } from './api/benefitClient'
...
// 注入 userStore 到 request 拦截器 (避免循环依赖)
// 必须在 pinia 注册之后调用, 否则 store 未初始化
setUserStoreGetter(() => useUserStore())

// benefit4j token 提供者: 从 store 读 (兼容基础级 localStorage + 推荐级内存)
setBenefitTokenProvider(() => useBenefitAuthStore().token)
