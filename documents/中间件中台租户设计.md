...
> **版本** v2.0 · **更新** 2026-08-28 · **状态** 定稿(长期标准,变更受 §1.0 演进规则约束) · **来源** benefit4j 实战抽象(见 §9 对照)
...
| 层 | 内容 | 稳定性 |
|---|---|---|
| **契约层**(本文档 §2~§7) | 租户模型(表 id 即租户)、三域接口、三级凭据、claim 命名、状态机、X-User-Id 语义、嵌入四入口 | **冻结**:字段只加不删不改名,语义只收紧不放松 |
...
| D9 | **SANDBOX 分级开通,配额代替审批** | 注册即全量(滥用面);人工审批(不可扩展,接入摩擦) | §6.2;Stripe/AWS/OpenAI 六家一致 |
| D10 | **验证通过前不发密钥** | 注册即返回(垃圾注册批量薅密钥) | §6.2 铁律 1 |
