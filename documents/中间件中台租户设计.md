...
> **版本** v1.3 · **更新** 2026-08-28 · **状态** 定稿 · **来源** benefit4j 实战抽象(见 §9 对照)
...
| 持久化 | 写 localStorage(iframe 内 SPA 跳转复用) | 不写,关 tab 即销毁 |
...
| 9 | 审计 | 平台域全部写操作 @Auditable 落审计表:**append-only(DB 权限 REVOKE UPDATE/DELETE)+ 每日 hash chain 防篡改**;密钥操作必审计 |
