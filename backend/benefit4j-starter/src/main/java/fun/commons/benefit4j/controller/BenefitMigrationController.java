...
/**
 * 一次性数据迁移控制器 (V1.2.2 app_secret 全量加密)
 * <p>
 * sensitive 模块上线后, app_secret 走 EncryptedFieldTypeHandler 自动加解密,
 * 但库里历史明文行读出会解密失败 → null。本接口用 JdbcTemplate 绕过 TypeHandler
 * 读原始值, 判断是否已加密, 对明文行用 updateById (走 TypeHandler) 加密回写。
 * <p>
 * 开关: {@code benefit4j.migration.app-secret-encrypt=true} (默认关), 运维手动开一次跑完即关。
 */
@RestController
@RequiredArgsConstructor
@RequiresToken(value = "OPS", type = "access")
@ConditionalOnProperty(prefix = "benefit4j.migration", name = "app-secret-encrypt", havingValue = "true")
public class BenefitMigrationController {
    ...
    @PostMapping("/benefit/api/v1/ops/migrate/encrypt-app-secret")
    public Object postMigrateEncryptAppSecret() {
        // 1. 绕过 TypeHandler 直读原始 app_secret (可能是明文或密文)
        List<Map<String, Object>> rows = jdbc.queryForList(
                "SELECT id, app_secret FROM ubma_application WHERE is_deleted = 0");
        int migrated = 0, skipped = 0, failed = 0;
        for (Map<String, Object> row : rows) {
            Long id = ((Number) row.get("id")).longValue();
            String raw = (String) row.get("app_secret");
            ...
            try {
                // 3. selectById 拿 entity (app_secret 字段读为 null 因明文解密失败),
                //    setAppSecret(原始明文) + updateById → TypeHandler 加密写入
                UbmaApplication app = applicationMapper.selectById(id);
                if (app == null) { failed++; continue; }
                app.setAppSecret(raw);
                applicationMapper.updateById(app);
                migrated++;
            } catch (Exception e) {
                log.warn("[Migration] app_secret 加密失败 id={}: {}", id, e.getMessage());
                failed++;
            }
        }
        ...
        log.info("[Migration] app_secret 加密完成: {}", result);
