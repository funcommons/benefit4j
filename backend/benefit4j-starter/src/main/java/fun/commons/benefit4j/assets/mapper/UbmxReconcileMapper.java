...
public interface UbmxReconcileMapper {
    ...
    @Select("/*traceid=assets,topic=reconcile_expect*/ "
            ...
            + "WHERE p.status = 'SUCCESS' AND p.tenant_id = #{tenantId} AND p.asset_code = #{assetCode}")
    BigDecimal expectedNormalBalance(@Param("tenantId") Long tenantId, @Param("assetCode") String assetCode);

    /** 实际值: NORMAL 户余额合计 */
    @Select("/*traceid=assets,topic=reconcile_actual*/ "
            + "SELECT COALESCE(SUM(balance), 0) FROM ubmx_account "
            + "WHERE tenant_id = #{tenantId} AND asset_code = #{assetCode} AND account_type = 'NORMAL'")
    BigDecimal actualNormalBalance(@Param("tenantId") Long tenantId, @Param("assetCode") String assetCode);
    ...
    @Select("/*traceid=assets,topic=reconcile_freeze*/ "
            ...
            + "FROM ubmx_account a WHERE a.tenant_id = #{tenantId} AND a.account_type = 'NORMAL' "
            + "AND a.frozen <> COALESCE((SELECT SUM(f.amount - f.used_amount) FROM ubmx_freeze f "
            + "WHERE f.account_id = a.id AND f.status = 'ACTIVE'), 0)")
    List<java.util.Map<String, Object>> freezeMismatches(@Param("tenantId") Long tenantId);
    ...
    @Insert("/*traceid=assets,topic=reconcile_snapshot*/ "
            ...
            + "WHERE tenant_id = #{tenantId} AND account_type = 'NORMAL' "
            + "ON CONFLICT (account_id, snap_date) DO UPDATE "
            + "SET balance = EXCLUDED.balance, frozen = EXCLUDED.frozen")
    int writeSnapshot(@Param("tenantId") Long tenantId);

    @Insert("/*traceid=assets,topic=reconcile_diff*/ "
            + "INSERT INTO ubmx_reconcile_diff (tenant_id, reconcile_type, asset_code, account_id, expected, actual) "
            + "VALUES (#{tenantId}, #{type}, #{assetCode}, #{accountId}, #{expected}, #{actual})")
    int insertDiff(@Param("tenantId") Long tenantId, @Param("type") String type,
