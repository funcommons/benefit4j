...
/**
 * 默认 HttpTransport 实现 (RestTemplate 同步) + 重试 (3 次, 500ms 间隔)。
 * <p>
 * 跨进程调用网络抖动自动重试, 3 次仍失败抛异常 (业务方 catch)。
 * 熔断由 Resilience4j yml 配置 (resilience4j.circuitbreaker.instances.benefit4jRemote)。
 */
