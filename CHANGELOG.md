...
内容与 2.0.0 完全一致,仅为规避 JitPack 失败构建缓存重发:首发构建踩中三个环境问题(JDK 8 默认 → 显式 openjdk17;仓根无 pom → `-f backend/pom.xml`;默认 Maven 过旧 → sdkman 装 3.9.9),`jitpack.yml` 落地后 v2.0.0 tag 的失败缓存无法 API 清除,发 2.0.1 使 tag 可拉取。
